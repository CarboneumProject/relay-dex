    ...
    if (txHash_dict !== null) {
      if (txHash_dict.length === 0) {
        ...
        Object.keys(txHash_dict).forEach(function (row) {
          ...
          idex.verifyTxHash(txHash).then(async (res) => {
            if (res) {
              let [walletAddress, wei_temp, tokenAddress] = res;
              let wei = new BigNumber(wei_temp).toFixed(0);
              let response = await hgetAsync('txHash:done:' + txHash, walletAddress.toLowerCase());
              if (response === '1') {
                logToFile.writeLog('loopDeposit', txHash + ' ' + walletAddress + ' have been deposited.');
              } else {
                let amount = await hgetAsync('txHash:new:' + txHash, walletAddress.toLowerCase());
